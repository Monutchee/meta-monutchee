SUMMARY = "MNCOS production flashing image for KR260Demo"
DESCRIPTION = "RAM-resident KR260Demo image that writes the production WIC to eMMC."
LICENSE = "MIT"
...
MNCOS_IMAGE_LABEL = "MNCOS KR260DEMO PRODUCTION FLASH IMAGE"

IMAGE_INSTALL = " \
    packagegroup-core-boot \
    systemd-conf \
    busybox \
    kr260demo-production-flasher \
"
...
KR260DEMO_PRODUCTION_FLASH_IMAGE_FSTYPES ?= "cpio.gz cpio.gz.u-boot"

python () {
    d.setVar("IMAGE_FSTYPES", d.getVar("KR260DEMO_PRODUCTION_FLASH_IMAGE_FSTYPES"))
}
...
# the RAM-resident KR260Demo implementation writes to eMMC.
do_copy_production_image() {
    install -d "${TFTPBOOT_DEST_DIR}"
    install -m 0644 \
        "${DEPLOY_DIR_IMAGE}/kr260demo-image-${MACHINE}.rootfs.wic.xz" \
        "${TFTPBOOT_DEST_DIR}/target.wic.xz"
    cd "${TFTPBOOT_DEST_DIR}"
    sha256sum target.wic.xz > target.wic.xz.sha256
}

do_copy_production_image[depends] += "kr260demo-image:do_image_complete"
