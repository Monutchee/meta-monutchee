...
class SetupSDKTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bitbake.touch()
        (self.workspace / "sources/meta-example/conf/templates/default").mkdir(parents=True)
        for product in ("kr260demo", "zuboard"):
            (self.workspace / f"sources/meta-monutchee/meta-{product}/conf/templates/default").mkdir(parents=True)

    def run_setup(self, product="example", build="build", shell="bash"):
        env = os.environ.copy()
    ...
    def test_all_products_use_split_core_and_explicit_bitbake(self):
        for product in ("example", "kr260demo", "zudemo"):
    ...
    def test_missing_product_template_fails(self):
        result = self.run_setup("missing")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("missing build template", result.stderr)
        self.assertNotIn("INITIALIZED", result.stdout)

    def test_ambiguous_product_template_fails(self):
        (self.workspace / "sources/meta-monutchee/meta-example/conf/templates/default").mkdir(parents=True)
        result = self.run_setup()
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("ambiguous build templates", result.stderr)
        self.assertNotIn("INITIALIZED", result.stdout)

    def test_external_template_is_exported(self):
        result = subprocess.run(
            ["bash", "-c", 'source ./setupSDK --product example && printf "TEMPLATE:%s\\n" "$TEMPLATECONF"'],
