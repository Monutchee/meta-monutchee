...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--product",
        required=True,
        help="lowercase product and Yocto machine identifier, for example example",
    )
    parser.add_argument(
        "--project-prefix",
        required=True,
        help="component repository/artifact prefix, for example EXAMPLE",
    )
    parser.add_argument(
        "--board",
        required=True,
...
def create_layer(args: argparse.Namespace) -> Path:
    ...
    try:
        ...
        for notice in ("LICENSING.md", "COMMERCIAL.md"):
            shutil.copy2(REPOSITORY_ROOT / notice, staging / notice)
            (staging / notice).chmod(0o644)
        shutil.copytree(REPOSITORY_ROOT / "LICENSES", staging / "LICENSES")
        for notice in (staging / "LICENSES").rglob("*"):
            if notice.is_file():
                notice.chmod(0o644)
