...
use the matching Scarthgap releases of OE-Core and BitBake, AMD/Xilinx
2026.1, and the community-layer revisions in each project manifest. Neither
...
Product image recipes independently select codec packages.
...
For each supported product, check fresh setup, layer resolution, both
headless values and invalid input, main/production-flash builds, SDK/eSDK
generation, and image/report contents. Check the Station artifact when enabled.
...
## Strict multimedia shutdown

Ordinary `MNCOS_HEADLESS = "1"` continues to preserve video capture and codecs.
The separate `MNCOS_MULTIMEDIA_OFF = "1"` setting removes those facilities and
audio as well, and coordinates ZynqMP boot-time hardware shutdown with the
Linux kernel and device tree. It defaults to `0` globally; products opt in explicitly.
It requires headless mode. Set it to `0` before setting `MNCOS_HEADLESS = "0"`.

The strict mode preserves AXI DMA/CMA/OpenAMP and the application acquisition path.
It does not disable the remote browser UI. Native/SDK tools and standalone
firmware multiconfigs retain their own features. The switch also selects APU-only
GPU slave ownership in the FSBL-linked xilpm configuration. Rebuild the entire image,
including FSBL and U-Boot, when changing the setting. See the
[shared hardware policy](../meta-zynqmp-addon/README.md#multimedia-shutdown).
